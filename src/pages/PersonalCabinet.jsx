...
const PersonalCabinet = () => {
  ...
  const loadBookingHistory = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.warn("Данные бронирований не являются массивом:", data)
        setBookings([])
      }
    } catch (error) {
      console.error("Ошибка загрузки бронирований:", error)
      setBookings([]) // Устанавливаем пустой массив при ошибке
  ...
  return (
    <div>
      ...
      <main className="container my-5 text-dark">
        <div className="row">
                    ...
                    <div key={booking.id} className={`card booking-card ${booking.status} mb-3`}>
                      <div className="card-body">
                        <div className="d-flex justify-content-between align-items-center mb-2">
                          <h5 className="card-title mb-0">Бронирование #{booking.id}</h5>
                          <span className={`badge bg-${getStatusClass(booking.status)}`}>
                            {getStatusText(booking.status)}
                          </span>
                        </div>
                        <p className="card-text">
                          <strong>Дата:</strong> {new Date(booking.start_time).toLocaleDateString("ru-RU")}
                          <br />
                          <strong>Время:</strong>{" "}
                          ...
                          <br />
                          <strong>Столик:</strong> {booking.table_details?.name}
                          <br />
                          <strong>Количество гостей:</strong> {booking.guests_count}
                          <br />
                          {booking.comment && (
                            <>
                            ...
                            <ul className="list-group list-group-flush">
                              {booking.menu_items.map((item, index) => (
                                ...
                                >
                                  ...
                                  <span>
                                    {item.quantity} x {item.price.toLocaleString()} сум
                                  </span>
                                </li>
                              ))}
                            </ul>
